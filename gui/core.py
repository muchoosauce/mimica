    ...
    *,
    ...
    if style_key and style_key != "realistic":
        ...
        if custom_ref_attached or (style_prefix and is_anchor_shot):
            # Triple-clause leak guard — see forge for full rationale.
            leak_guard = (
                "===  CRITICAL RULES — READ THESE FIRST  ===\n\n"
                "🎨 STYLE TRANSFER ONLY: the first reference image "
                "attached is a STYLE SAMPLE. You will copy from it "
                "ONLY the rendering technique (medium, subsurface "
                "scattering, materials, lighting, color grading, "
                "shading register, bokeh quality, render quality). "
                "You will NOT copy its subject, composition, scene, "
                "characters, people, faces, framing, layout, or "
                "environment.\n\n"
                "🚫 FORBIDDEN IN THE OUTPUT FRAME (even if the "
                "reference image suggests them): a family, a group "
                "of people, multiple humans together, a parent + "
                "child scene, a living room interior with characters, "
                "a sofa with people, a board game, characters playing "
                "together, a generic indoor lifestyle scene, a portrait "
                "of multiple people. The reference image happens to "
                "show those things ONLY as a style swatch — they are "
                "NOT what you generate.\n\n"
                "✅ THE OUTPUT MUST SHOW: EXACTLY and ONLY what the "
                "SCENE block below describes. If the SCENE describes "
                "an unusual, surreal, or anthropomorphic subject (a "
                "face emerging from skin, a sentient object, a close-"
                "up of a material, a single hand, an abstract form, "
                "a body part with a face on it, a non-human subject), "
                "render THAT exact subject. The reference image's "
                "human composition is IRRELEVANT to the output content.\n\n"
                "Preserve every framing constraint from SCENE verbatim "
                "(e.g. \"thigh only, no hip, no crotch\" means exactly "
                "that — only the thigh, no other body part).\n\n"
                "===  END CRITICAL RULES  ==="
            )
            descriptor = f"\n\nAESTHETIC NOTES (text-only reinforcement of the style): {style_prefix}" if style_prefix else ""
            refined_prompt = f"{leak_guard}{descriptor}\n\nSCENE TO RENDER: {refined_prompt}".strip()
        elif style_prefix:
            refined_prompt = (
